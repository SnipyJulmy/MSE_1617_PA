...
it, so we need another library which had this idea of port.
...
code. Some existing product already use Draw2d for diagrams manipulations :
...
is lacking the idea of port, but Draw2d already had them.
...
The listing \ref{lst:draw2d-base-graph-model} is producing the HTML page shown in
...
\item Nodes own ports, so we need to layout the edges on specific part of the graph
\end{itemize}
